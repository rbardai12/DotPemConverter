...
def EnterPath():
    ...
    newname = re.sub('.cer', '', newname)

    txtline2 = f'x509 -inform DER -outform PEM -in "{nameoffile}" -out "{newname}.pem"'
...
def box2command():
    ...
    fileout1 = 'CAfile1.cer'
    fileout2 = 'CAfile2.cer'
    fileout3 = 'CAfile3.cer'
        ...
        for line in infile:
            ...
            if counter == 1:
                if line.strip() == "-----BEGIN CERTIFICATE-----":
                    ...
                    outfile2.write(line)
    #CAfile1 REWRITE
    with open('CAfile1.cer', "r+") as f: 
        a = f.read() 
        with open('CAfile1.cer', "w+") as f: 
    ...
    #CAfile2 REWRITE
    with open('CAfile2.cer', "r+") as f: 
        a = f.read() 
        with open('CAfile2.cer', "w+") as f: 
            f.write("-----BEGIN CERTIFICATE-----")
            f.write('\n')
            f.write(a)
            with open('CAfile2.cer', "r+") as f: 
    ...
    output1 = Variable.cerCAfile1path
    output1 = re.sub('.cer', '', output1)
    codecontent1 = f'x509 -inform DER -outform PEM -in "{Variable.cerCAfile1path}" -out "{output1}.pem"'
    ...
    time.sleep(2)
    output2 = Variable.cerCAfile2path
    output2 = re.sub('.cer', '', output2)
    codecontent2 = f'x509 -inform DER -outform PEM -in "{Variable.cerCAfile2path}" -out "{output2}.pem"'
...
def box3command():
    ...
    with open('CAfile2.cer', "r+") as f: 
        ...
        with open('CAfile2.cer', "w+") as f: 
            ...
            with open('CAfile2.cer', "r+") as f: 
                b = f.read() 
    #CAfile3 REWRITE
    with open('CAfile3.cer', "r+") as f: 
        a = f.read() 
        with open('CAfile3.cer', "w+") as f: 
            f.write("-----BEGIN CERTIFICATE-----")
            f.write('\n')
            f.write(a)
            with open('CAfile3.cer', "r+") as f: 
    ...
    keyboard.press(Key.enter)
    time.sleep(2)
    output3 = Variable.cerCAfile3path
    output3 = re.sub('.cer', '', output3)
    codecontent3 = f'x509 -inform DER -outform PEM -in "{Variable.cerCAfile3path}" -out "{output3}.pem"'
...
#Top Label
l = tk.Label(root, text='Type in the path to the .cer Certificate')
l.pack()
