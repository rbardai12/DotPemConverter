...
def box2command():
    ...
    fileout1 = 'CAfile1.crt'
    fileout2 = 'CAfile2.crt'
    fileout3 = 'CAfile3.crt'
        ...
        for line in infile:
            ...
            if counter == 1:
                if line.strip() == "-----BEGIN CERTIFICATE-----":
                    ...
                    outfile2.write(line)
    #CAfile1 REWRITE
    with open('CAfile1.crt', "r+") as f: 
        a = f.read() 
        with open('CAfile1.crt', "w+") as f: 
    ...
    #CAfile2 REWRITE
    with open('CAfile2.crt', "r+") as f: 
        a = f.read() 
        with open('CAfile2.crt', "w+") as f: 
            f.write("-----BEGIN CERTIFICATE-----")
            f.write('\n')
            f.write(a)
            with open('CAfile2.crt', "r+") as f: 
    ...
    output1 = Variable.crtCAfile1path
    output1 = re.sub('.crt', '', output1)
    codecontent1 = f'x509 -inform DER -outform PEM -in "{Variable.crtCAfile1path}" -out "{output1}.pem"'
    ...
    time.sleep(2)
    output2 = Variable.crtCAfile2path
    output2 = re.sub('.crt', '', output2)
    codecontent2 = f'x509 -inform DER -outform PEM -in "{Variable.crtCAfile2path}" -out "{output2}.pem"'
...
def box3command():
    ...
    with open('CAfile2.crt', "r+") as f: 
        ...
        with open('CAfile2.crt', "w+") as f: 
            ...
            with open('CAfile2.crt', "r+") as f: 
                b = f.read() 
    #CAfile3 REWRITE
    with open('CAfile3.crt', "r+") as f: 
        a = f.read() 
        with open('CAfile3.crt', "w+") as f: 
            f.write("-----BEGIN CERTIFICATE-----")
            f.write('\n')
            f.write(a)
            with open('CAfile3.crt', "r+") as f: 
    ...
    keyboard.press(Key.enter)
    time.sleep(2)
    output3 = Variable.crtCAfile3path
    output3 = re.sub('.crt', '', output3)
    codecontent3 = f'x509 -inform DER -outform PEM -in "{Variable.crtCAfile3path}" -out "{output3}.pem"'
...
#Top Label
l = tk.Label(root, text='Type in the path to the .crt Certificate')
l.pack()
