...
def EnterPath():
    ...
    newname = re.sub('.pfx', '', newname)

    txtline2 = f'pkcs12 -in "{nameoffile}" -nokeys -out "{newname}.pem"'
...
def box2command():
    ...
    fileout1 = 'CAfile1.pfx'
    fileout2 = 'CAfile2.pfx'
    fileout3 = 'CAfile3.pfx'
        ...
        for line in infile:
            ...
            if counter == 1:
                if line.strip() == "-----BEGIN CERTIFICATE-----":
                    ...
                    outfile2.write(line)
    #CAfile1 REWRITE
    with open('CAfile1.pfx', "r+") as f: 
        a = f.read() 
        with open('CAfile1.pfx', "w+") as f: 
    ...
    #CAfile2 REWRITE
    with open('CAfile2.pfx', "r+") as f: 
        a = f.read() 
        with open('CAfile2.pfx', "w+") as f: 
            f.write("-----BEGIN CERTIFICATE-----")
            f.write('\n')
            f.write(a)
            with open('CAfile2.pfx', "r+") as f: 
    ...
    output1 = Variable.pfxCAfile1path
    output1 = re.sub('.pfx', '', output1)
    codecontent1 = f'pkcs12 -in "{Variable.pfxCAfile1path}" -nokeys -out "{output1}.pem"'
    ...
    time.sleep(2)
    output2 = Variable.pfxCAfile2path
    output2 = re.sub('.pfx', '', output2)
    codecontent2 = f'pkcs12 -in "{Variable.pfxCAfile2path}" -nokeys -out "{output2}.pem"'
...
def box3command():
    ...
    with open('CAfile2.pfx', "r+") as f: 
        ...
        with open('CAfile2.pfx', "w+") as f: 
            ...
            with open('CAfile2.pfx', "r+") as f: 
                b = f.read() 
    #CAfile3 REWRITE
    with open('CAfile3.pfx', "r+") as f: 
        a = f.read() 
        with open('CAfile3.pfx', "w+") as f: 
            f.write("-----BEGIN CERTIFICATE-----")
            f.write('\n')
            f.write(a)
            with open('CAfile3.pfx', "r+") as f: 
    ...
    keyboard.press(Key.enter)
    time.sleep(2)
    output3 = Variable.pfxCAfile3path
    output3 = re.sub('.pfx', '', output3)
    codecontent3 = f'pkcs12 -in "{Variable.pfxCAfile3path}" -nokeys -out "{output3}.pem"'
...
#Top Label
l = tk.Label(root, text='Type in the path to the .pfx Certificate')
l.pack()
